...
#define RELINOW_RELIABLE_TX_PERIOD_MS 3000u
#define RELINOW_UNRELIABLE_BURST_PERIOD_MS 5000u
#define RELINOW_UNRELIABLE_BURST_COUNT 3u
#define RELINOW_PRIORITY_BURST_PERIOD_MS 1700u
...
void app_main(void) {
    ...
    while (1) {
        ...
        if (now >= startup_guard_until && (now - last_priority_burst) >= RELINOW_PRIORITY_BURST_PERIOD_MS) {
            char stale_msg[96];
            char fresh_msg[96];
            int n_stale = snprintf(stale_msg, sizeof(stale_msg), "B->A priority stale #%lu", (unsigned long)priority_counter);
