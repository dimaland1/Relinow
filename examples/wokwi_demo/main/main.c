...
void app_main(void) {
    ...
    relinow_config_t rn_cfg;
    relinow_default_config(&rn_cfg);
    ...
    ESP_ERROR_CHECK(relinow_init(&g_node, &rn_cfg));
    
    ESP_LOGI(TAG, "=== DEMO SCENARIO START ===");
    
    // 1. Phase UNRELIABLE
    ESP_LOGI(TAG, "Phase 1: UNRELIABLE burst");
    relinow_open_channel(&g_node, 10, RELINOW_MODE_UNRELIABLE, 10);
    for (int i=0; i<5; i++) {
        relinow_send_unreliable(&g_node, 10, (uint8_t*)"RawData", 7);
    }
    
    // 2. Phase RELIABLE
    ESP_LOGI(TAG, "Phase 2: RELIABLE critical message");
    relinow_open_channel(&g_node, 20, RELINOW_MODE_RELIABLE, 10);
    relinow_send_reliable(&g_node, 20, (uint8_t*)"Hello Wokwi", 11, xTaskGetTickCount() * portTICK_PERIOD_MS);
    
    // 3. Phase MULTIPLEX
    ESP_LOGI(TAG, "Phase 3: MULTIPLEXING multiple reliable channels");
    relinow_open_channel(&g_node, 21, RELINOW_MODE_RELIABLE, 10);
    relinow_open_channel(&g_node, 22, RELINOW_MODE_RELIABLE, 10);
    relinow_send_reliable(&g_node, 21, (uint8_t*)"Chan 21 msg", 11, xTaskGetTickCount() * portTICK_PERIOD_MS);
    relinow_send_reliable(&g_node, 22, (uint8_t*)"Chan 22 msg", 11, xTaskGetTickCount() * portTICK_PERIOD_MS);

    // 4. Phase HEARTBEAT (Wait for timeout)
    ESP_LOGI(TAG, "Phase 4: Waiting for heartbeat timeout...");
    while (1) {
        relinow_poll(&g_node, xTaskGetTickCount() * portTICK_PERIOD_MS);
